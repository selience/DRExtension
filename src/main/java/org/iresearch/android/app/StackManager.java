...
import org.iresearch.android.base.BaseFragment;
import org.iresearch.android.base.XBaseFragment;
...
public final class StackManager {
    ...
    private final Stack<XBaseFragment> mStack;
    ...
    public void replace(Class<? extends XBaseFragment> clazz, String tag, Bundle args) {
        XBaseFragment f = (XBaseFragment) mFragmentManager.findFragmentByTag(tag);
     ...
     * 获取Fragment实例
     */
    private XBaseFragment getFragment(Class<? extends BaseFragment> clazz, String tag, Bundle args) {
        XBaseFragment f = (XBaseFragment) mFragmentManager.findFragmentByTag(tag);
        if (f == null || !f.isSingleton()) {
            f = (XBaseFragment) Fragment.instantiate(mFragmentActivity, clazz.getName(), args);
        }
        return f;
    }
     ...
     * 处理栈顶Fragment
     */
    private boolean resetFragment(XBaseFragment f, String tag) {
     ...
     * 清空栈顶Fragment
     */
    private void cleanBackStack(XBaseFragment f) {
        if (f.isCleanStack()) {
            while (mStack.size() > 0) {
                synchronized (lock) {
                    mStack.pop();
    ...
    public void savedState(Bundle outState) {
        ...
        Iterator<XBaseFragment> iterator = mStack.iterator();
    ...
    public void restoreState(Bundle outState) {
        if (outState != null) {
            ...
            for (String tag : stackTags) {
                mStack.add((XBaseFragment) mFragmentManager.findFragmentByTag(tag));
            }
        }
    }
     ...
     * 添加Fragment到栈顶
     */
    private void addFragment(XBaseFragment f) {
        synchronized (lock) {
            mStack.add(f);
        }
    }
     ...
     * 获取栈顶Fragment
     */
    public XBaseFragment peekFragment() {
        synchronized (lock) {
            return mStack.peek();
        }
    }
